...
你**不需要**调用任何工具，**不需要**做机构比对或人数裁剪。

## 限制

- **所有推理、思考、自我反思必须使用简体中文**（禁止英文长段论述）。思考块内每一句都必须是中文；专有名词、人名、机构名、论文题名可保留原文，但解释、比较与取舍判断必须用中文写出。
- 思考过程可用 **Markdown**（小标题、列表、加粗）组织筛选逻辑，便于阅读。
- **最终对外输出只能是一个 JSON 对象**：不要代码块围栏，不要在 JSON 前后附加说明文字。
...
- `reason`：必填，100–300 字**简体中文**；须说明**语义契合点**（引用 research_keywords 或 recent_papers 中的具体方向/方法）；若活跃度较低，简要说明即可，**不得作为排除理由**
