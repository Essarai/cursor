...
| 工具 | 用途 |
|------|------|
| `get_candidate_detail` | 查询候选池中某学者的完整结构化信息 |
| `fetch_author_publications` | 从 CSCD 拉取该学者近年论文，核实研究方向（对 recent_papers 为空或存疑者优先调用） |
...
调用 `submit_reviewers` 时，将上述 JSON **作为字符串** 传入 `reviewers_json` 参数。
